...
#define DEBUG_ENABLE                       1   // True for showing debug messages
#if DEBUG_ENABLE
#define DEBUG        printf
#else
#define	DEBUG(...)
...
#define PROLIFIC_VID                  0x0ea0   // Prolific Vender ID
#define PL2701_PID                    0x7301   // Prolific Product ID
...
typedef	struct _DEV_STATUS
{
	unsigned char remotewakecapable : 1;     // Remote device is running at super speed (USB 3.0) when it's 1 
	unsigned char remotewakeup : 1;   // Remote device was suspend when it's 1
} DEV_STATUS;
